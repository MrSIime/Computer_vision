import cv2, os
import numpy as np
from sklearn.model_selection import train_test_split
from sklearn.neighbors import KNeighborsClassifier


def generate_image(color, shape):
    img = np.zeros((200, 200, 3), np.uint8)

    if shape == "circle":
        cv2.circle(img, (100, 100), 50, color, -1)
    elif shape == "square":
        cv2.rectangle(img, (50, 50), (150, 150), color, -1)
    elif shape == "triangle":
        points = np.array([[100, 40], [40, 160], [160, 100]])
        cv2.drawContours(img, [points], 0, color, -1)
    return img

x = []
y = []

colors = {
    'red': (0, 0, 255),
    'green': (0, 255, 0),
    'blue': (255, 0, 0),
    'yellow': (0, 255, 255),
    'pink': (203, 192, 255),
    'purple': (128, 0, 128),
    'orange': (0, 165, 255),
    'cyan': (255, 255, 0)
}

shapes = ['circle', 'square', 'triangle']

for color_name, bgr in colors.items():
    for shape in shapes:
        for _ in range(10):
            img = generate_image(bgr, shape)
            mean_color = cv2.mean(img)[:3]
            features = [mean_color[0], mean_color[1], mean_color[2]]

            x.append(features)
            y.append(f'{color_name}_{shape}')

x_train, x_test, y_train, y_test = train_test_split(x, y, test_size = 0.3)

model = KNeighborsClassifier(n_neighbors=3)
model.fit(x_train, y_train)

accuracy = model.score(x_test, y_test)

history = []
max_history = 5

def predict_with_smoothing(current_features):
    global history
    history.append(current_features)
    
    if len(history) > max_history:
        history.pop(0)
    
    smoothed_features = np.mean(history, axis=0)
    
    prediction = model.predict([smoothed_features])[0]
    probabilities = model.predict_proba([smoothed_features])[0]
    confidence = np.max(probabilities)
    
    return prediction, confidence

for _ in range(5):
    res, prob = predict_with_smoothing(features)

print(f"Прогноз: {res} | Впевненість: {round(prob*100, 2)}%")
print(f"точність моделі : {round(accuracy*100, 2)}%")

test_img = generate_image((0, 255, 0), 'circle')
mean_color = cv2.mean(img)[:3]
features = model.predict([mean_color])

print(f"модель каже це {features}")

cv2.imshow("test", test_img)
cv2.waitKey()
cv2.destroyAllWindows